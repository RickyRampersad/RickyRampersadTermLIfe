/* ============================================================
   ...
   ============================================================ */
function getVersion(){ return 'v6.9-COLOR-CODE'; }
...
function doPost(e) {
  try {
    ...
    const reference = 'RRB/' + year + '/' + pad(runNo) + '/' + clean(d.client||d.name) + '/' + clean(subjPart);
...
function sendRoutedEmail(d) {
  ...
  +     row('Reporting manager', mgrName)
  +     row('Client contact', d.email + (d.phone ? ' &middot; '+d.phone : ''))
  +     attachNote
  +   '</table>'
  ...
  +   '<div style="background:#f6fafd;border:1px solid #e4eef6;border-left:3px solid '+col.hex+';border-radius:0 10px 10px 0;padding:14px 16px;'+F+'font-size:13.5px;color:#0a2138;line-height:1.6;white-space:pre-wrap;">'+ (d.description||'').replace(/</g,'&lt;') +'</div>'
