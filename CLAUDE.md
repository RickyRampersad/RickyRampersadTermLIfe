...
`/vid/` is the film alone, for WhatsApp, and `/templates/` is every letter on
one page as the client receives it — generated by `build-letters.py` with the
letters, never edited by hand; `tools/film/chapters.py` walks `/vid/` and
