...
(async () => {
  ...
  await page.goto(`http://localhost:${PORT}/newday/`, { waitUntil: 'networkidle' });
  ...
  const film = path.join(ROOT, FILM, 'the-new-day.mp4');
  ...
  // The last line here is a long one — the address, read out — so the tail
  // after the last mark is that line plus the beat of air, not two seconds.
  const tail = dur - marks[marks.length - 1].start;
  ok('and the film runs as long as the narration does', tail > 2 && tail < 15,
     'film ' + dur + 's, last mark ' + marks[marks.length - 1].start + 's');

  console.log('\nA mark seeks:\n');
  const keep = [2, 3, 4, 5, 6, 7, 9];   // the same set the page keeps
  ...
  const text = fs.readFileSync(path.join(ROOT, 'newday/index.html'), 'utf8') +
