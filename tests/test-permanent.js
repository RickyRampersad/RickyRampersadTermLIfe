...
//
//   LIBERATOR's premium runs to 65, 75, 85 or 100, and one issued 2009 or
//   earlier CAN SIMPLY BE EXTENDED. No application, no underwriting. That is
//   the one thing on this screen an agent can act on this week.
//
...
const due = (num, plan, sum, prem, st2, desc) =>
  ['Anand Pretend', num, '1', 'CLIENTNAME', prem, '2005-01-01', '0', st2 || '', 0, '2',
...
ok('the extension year is the branch’s, not a guess', d.extendYear === 2009);
...
ok('and their cover is not in the total', d.extend.cover === 8000000,
   String(d.extend.cover) + ' (4m + 3m + 1m, and not the excluded 8m or the lapsed 9m)');
ok('nor their policy', d.extend.n === 3, String(d.extend.n));
...
ok('the extension list is cut at the branch’s year',
   /ISSUE_DATE__c < 2010-01-01/.test(all));
