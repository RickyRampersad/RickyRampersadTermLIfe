#!/usr/bin/env bash
# Assemble the KPI tracker training film.
#
#   ./build-voice.sh     # narration first (Andrew, edge-tts)
#   ./build-film.sh      # then this
#
# Plates are 1920x1080 stills rendered from the real app — real screenshots of
# a week that was actually reported block by block, not mock-ups. Each plate is
# held for exactly as long as its narration line, with a beat of air after it,
# and given a very slow push in so the frame is not dead.
#
# The branch theme sits underneath at -22 dB, looped and faded. It is a bed,
# not a score: if you can name the tune while Andrew is talking, it is too loud.
set -euo pipefail
cd "$(dirname "$0")"

PLATES="${PLATES:-plates}"          # 01.png … 20.png
MUSIC="${MUSIC:-../../rr-branch-theme.mp3}"
OUT="${OUT:-kpi-tracker.mp4}"
GAP=0.75                            # seconds of air after each line
FPS=25
WORK="$(mktemp -d)"
...
for f in line*.mp3; do
  ...
  # a slow push in — 4% over the whole hold
  ffmpeg -y -v error -loop 1 -i "$plate" \
    -filter_complex "scale=2400:-1,zoompan=z='min(1+0.04*on/$frames,1.04)':d=$frames:\
x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1920x1080:fps=$FPS,format=yuv420p" \
    -t "$seg" -c:v libx264 -preset slow -crf 24 -pix_fmt yuv420p "$WORK/v$n.mp4"
  echo "file '$WORK/v$n.mp4'" >> "$WORK/vlist.txt"

  # the line, then the beat of air
...
echo "Laying the bed"
# Loop the theme to cover, drop it well under the voice, fade both ends.
