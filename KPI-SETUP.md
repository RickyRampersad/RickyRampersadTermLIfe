...
### The register is written AFTER the session is handed over

**Signing in does not write to the workbook, and that is deliberate.**

On 14 September 2026 a formula in the portfolio tab exceeded Google's own
limits — *"Query has exceeded the available resources"* — so the workbook
stopped recalculating. Sign-in was the only thing on the tracker that
**wrote** to it: it stamped the attendance register before it answered. Two
people lost a morning to *"The sheet did not answer, after 4 tries over about
two minutes"*, while a rejected password came back in **0.7s** and every read
on the same workbook answered in under four.

A `try/catch` was no protection. **A write that hangs never throws**, so the
browser waited its 35 seconds, retried four times, and gave up.

So the path is split in two:

| Step | What it does | If it fails |
|---|---|---|
| `login` / `me` | hands over the token, profile, roster and schedule — every one a read — and returns `attendance: null` | the person cannot sign in, and is told which of the two reasons it was |
| `register` | `registerDay_()` writes the attendance row and returns it | **nothing stops.** The person is already working; the attendance line stays blank until the next reload |

The browser calls `register` once it is in and never waits on it. The
"first sign-in of the day opens on the plan" rule already fell back to the
clock when the server said nothing, so it needed no change.

`e2e-login.js` breaks the register on purpose and asserts she still gets in,
that sign-in was **one** attempt rather than four, and that nothing about a
sheet that did not answer reaches her screen. `test-attendance.js` asserts
the same contract at the function level.

### And signing in does not READ the spreadsheet either

The register was half the problem. The other half was the Access tab: every
sign-in read it to find the person. So the same fix went the other way —
**the sheet is off the sign-in path in both directions.**

There were five sign-in failures in twelve days, each patched where it showed:
...
| 10 Sep, 7:15am | three retries were not enough and the branch was locked out |
| 14 Sep | the workbook could not recalculate, so the register write hung |

What they share was never fixed: **signing in depended on a Google Sheet,
synchronously, at that instant.** Now it doesn't.

**The roster mirror.** The Access tab stays the only place a password is
edited, but a mirror of it lives in the script's own properties, and that is
what a sign-in reads. The happy path is a property read and a hash
comparison — no Sheets call at all, nothing to queue behind a trigger,
nothing that can hang on a recalculation.

- **The mirror holds no passwords**, only a salted SHA-256 of each, so a
  second copy of the branch's credentials does not exist anywhere. The salt
  is made once and kept in the same properties.
- **`keepWarm` refreshes it every ten minutes**, riding on the trigger that
  already existed to wake the workbook. No new trigger — which matters,
  because a project gets twenty and this one uses eighteen.
- **A miss asks the sheet for a second opinion.** A password changed five
  minutes ago would fail against a ten-minute-old mirror, so a login that
  misses refreshes once and asks again before refusing anybody. A miss is the
  rare path and nobody's morning depends on it. If the sheet cannot be read
  even then, the mirror's answer stands rather than a timeout.
- **`refreshRosterMirror()`** refreshes by hand from the editor when somebody
  has just changed a password and will not wait ten minutes.

`test-attendance.js` counts the reads: it wraps `accessSheet_` and asserts a
successful sign-in opens the Access tab **zero** times, that a wrong password
is the only path that asks the sheet again, and that what is stored is a hash
