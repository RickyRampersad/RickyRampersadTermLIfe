...
var SB = { navy: '#4A3570', blue: '#6D4AA8', gold: '#E8A33D', light: '#F0EAFC', ink: '#3B2A5C' };
...
function wrap_(inner, tag, id) {
  ...
  var bg = id.dhaa ? '#4A3570' : SB.navy;
  var chip = id.dhaa ? '#F0EAFC' : SB.gold;
  var chipInk = id.dhaa ? '#4A3570' : SB.navy;
...
function sendClientThanks_(ref, priority, body, formPdf, letterPdf, accessCode) {
  ...
  if (id0.dhaa && accessCode) {
    ...
    track = box_('tip',
      ...
      '<div style="font-size:22px;font-weight:800;letter-spacing:.22em;margin:10px 0 4px;color:#4A3570">' +
      esc_(accessCode) + '</div>' +
...
function routeToService_(ref, priority, now, body, attachments, clientEmailed) {
  ...
  (body.fields || []).forEach(function (f) {
    ...
    if (f.flag === 'agent')   actions.push(badge_('AGENT', '#4A3570') + '<b>' + esc_(f.label) + '</b> — ' + esc_(f.value));
...
function matchAssignmentForRow_(sh, row, agentName, agentNo, why) {
  ...
  var refInk = id.dhaa ? '#4A3570' : SB.navy;
  var now = new Date();
...
function clientPatiencePass_() {
  [SVC.IND_SHEET, SVC.GRP_SHEET].forEach(function (name) {
    ...
    rows.forEach(function (r, idx) {
      ...
      var refInk = rowId.dhaa ? '#4A3570' : SB.navy;
...
function cta_(label, url, dhaa) {
  return '<table cellpadding="0" cellspacing="0" style="margin:18px 0 6px"><tr>' +
    '<td style="background:' + (dhaa ? '#4A3570' : SB.navy) + ';border-radius:8px">' +
