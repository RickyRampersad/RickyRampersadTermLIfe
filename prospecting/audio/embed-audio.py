...
The film has to stay a single file: the branch wall plays it in an iframe and
the hosted copy will not fetch a sibling asset, so an <audio src="line01.mp3">
would be silent in exactly the two places it matters.
...
print(f'{film.name} is now {len(html)/1024:.0f}KB.')
